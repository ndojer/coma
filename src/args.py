...
class Args(NamedTuple):
      ...
      outputMode: Literal["separate", "single", "split-alignments"]
      ...
      def parse(args: List[str] = None) -> Args:
            ...
            parser.add_argument("-oM", "--outputMode", dest="outputMode", type=str,
                              default="single", choices=["separate", "single", "split-alignments"],
                              help=("Mode used while creating output alignment file(s): "
                                    "    'single' - single-pass running mode (default); "
                                    "    'separate' - returns 1st-pass alignments on stdout and writes subsequent passes to extra files; "
                                    "    'split-alignments' - returns a single file with all multi-pass alignments."))

            parser.add_argument("-sr", "--scalingRange", dest="scalingRange", type=float, default=0,
                                    help="The extent to which a molecule can be stretched or compressed relative to its length (default: no stretching).")

            parser.add_argument("-st", "--scalingStep", dest="scalingStep", type=int, default=None,
                                    help="Stretching step of query molecule scaling (default: equals to parameter primaryResolution). ")
            ...
            parser.add_argument("-sp", "--perfectMatchScore", dest="perfectMatchScore", type=int, default=1000,
                                    help=("Score value given to an aligned pair with 0 distance between reference and query positions."))

            parser.add_argument("-dp", "--distancePenaltyMultiplier", dest="distancePenaltyMultiplier", type=float, default=1.,
                                    help=("Multiplier applied to the distance between reference and query positions "
            ...
            parser.add_argument("-a", "--benchmarkAlignment", dest="benchmarkAlignmentFile", type=argparse.FileType("r"),
                                    default=None,
                                    help=("XMAP file containing alignments from other source, to be used with 'diagnostics' option."))
            ...
            parser.add_argument("-img", "--inv_max_gap", dest="inv_max_gap", type=int, default=2_000_000,
                                    help="Maximum allowed reference gap for single-breakpoint inversion (bp). Default: 2,000,000.")
