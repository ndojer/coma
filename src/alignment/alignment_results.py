...
class SmapResultRow:
    ...
    @classmethod
    def build_translocation_entry(cls, qid, A, B, base_type: str, mol_orientation: str) -> "SmapResultRow":
        a_qlo, a_qhi = cls.qry_iv(A)
        b_qlo, b_qhi = cls.qry_iv(B)
        if a_qlo > b_qlo:
            A, B = B, A
            a_qlo, a_qhi, b_qlo, b_qhi = b_qlo, b_qhi, a_qlo, a_qhi
        ...
        a_inv = (cls.ori_char(A) != mol_orientation)
        b_inv = (cls.ori_char(B) != mol_orientation)

        ref_A = float(cls.ref_at_qlo(A) if a_inv else cls.ref_at_qhi(A))
        ref_B = float(cls.ref_at_qhi(B) if b_inv else cls.ref_at_qlo(B))
        r.data["RefStartPos"] = ref_A
        r.data["RefEndPos"]   = ref_B

        r.data["Type"] = str(base_type)
        r.data["SVsize"] = -1.0
        r.data["Confidence"] = 0.99
        r.data["XmapID1"] = cls.get_xmap_id(A)
        r.data["XmapID2"] = cls.get_xmap_id(B)
        r.data["Orientation"] = cls.pair_orientation(A, B)
    ...
    def from_alignment_group(cls, qid: int, group_rows: List[Any]) -> List["SmapResultRow"]:
        ...
        for i in range(n - 1):
            ...
            if t is None:
                continue
            row = cls.build_translocation_entry(qid, A, B, base_type=t, mol_orientation=mol_orientation)
            row.data["LinkID"] = -1
            out.append(row)
